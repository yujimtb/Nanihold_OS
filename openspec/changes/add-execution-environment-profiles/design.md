...
- **環境契約(EnvironmentContract)**: 「何が満たされていればよいか」の宣言。共通要件(シェル、workspace 書き込み可、最低メモリ、sandbox、パス論理名)と、`adapters` に列挙するアダプタ別要件(endpoint、任意の最低CLI版)の集合で表す。ポータブルで機械非依存。オーナー(S5)が承認するのはこの契約と調達ポリシーの境界(使用可能な資源・ネットワーク・予算)のみ。
...
環境が満たすべき能力を機械非依存で表す。共通要件とアダプタ別要件の2層で構成する。

- 共通要求能力: 1つ以上のシェル種別(`supported_shells`)のいずれか、workspace 書き込み可、最低メモリ等。
- アダプタ別要求能力: `adapters[adapter].required_endpoints` の疎通と、必要な場合の `adapters[adapter].minimum_cli_version`。契約は map に列挙した全アダプタをホストできることを宣言する。
- `supported_sandboxes` / 要求 sandbox モード: この契約が要求し、実体が実際に提供できるべき sandbox モード(例: `["workspace-write", "read-only"]`)。
- 契約全体の endpoint 集合は adapter map の `required_endpoints` の和集合から導出する。単一の契約全体 endpoint/version フィールドは持たない。
- パス写像の論理名: 例 `workspace-root`。物理パスではなく論理名のみを宣言する。

環境契約は具体的な実行場所の名前(`windows-native` 等)を identity 要素として列挙しない。`environment_fingerprint` はこの環境契約の正規化ハッシュとして定義する。アダプタ map のキー順序と各 endpoint tuple の順序は正規化してからハッシュする。機械固有情報・実体識別情報は一切含めない。スケール時は環境契約を LETHE に版付きアーティファクトとして保存し、PilotHost が起動時にコントロールプレーンから取得する。
...
preflight は環境実体が環境契約に適合するかを実測で確かめる契約適合テストである。dispatch 対象の adapter を 1 回試走させ、生成された rollout の `sandbox_policy` を読み、共通要件とその adapter の endpoint・最低CLI版を突き合わせる。契約に無い adapter は試走せず fail-closed とする。
...
- 検証タプル(adapter × CLI バージョン × sandbox モード × `environment_fingerprint`)が前回検証済みタプルと一致すれば、その adapter の preflight をスキップ(キャッシュヒット)。
...
- **契約全体 endpoint/version フィールドを廃止**: Codex 固有の単一フィールドを残すと Claude Code や将来の CLI を宣言できないため、共通能力と adapter map を別層にし、全体 endpoint は map の和集合から導出する。これにより契約の正規化と preflight の対象選択を同じ宣言から決定できる。
