...
- 追加指示は `instruction_received` イベント + 対象 Node への `INSTRUCTION` Message としてFIFO配送する。
  対象 Node は次の LLM invocation 開始前に未適用分を全件プロンプトへ注入し、指示ごとの
  `instruction_applied` に適用先 invocation ID を記録する。実行中 invocation は割り込まない。
