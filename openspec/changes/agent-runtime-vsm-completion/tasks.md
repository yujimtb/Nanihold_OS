...
## Wave 3: トークン削減(§4)
- [x] 3.1 `vsm/memory/builder.py` ContextViewBuilder
- [x] 3.2 S1 完了時の TaskSummary 生成 + TaskSummaryIndex 登録
- [x] 3.3 NodeRunState.session_refs、Run内・Node内セッション再開、resume 失敗時の新規フォールバック
- [x] 3.4 テスト(ビュー構築の決定論、resume 経路、フォールバック)

## Wave 4: S2 AI 調停 + Algedonic + Consortium(§7, §8, §9)
- [x] 4.1 S2Coordinator の AI 調停ハンドラ(設定でオンオフ)
- [x] 4.2 ChannelId.ALGEDONIC + raise_algedonic facade + S5 ハンドラ + Human 通知フック
- [x] 4.3 `vsm/runtime/consortium.py`(参加者=Node参照+Human、ラウンド制、タイムアウトポリシー)
- [x] 4.4 招集トリガ3種(S5 任意 / algedonic 連動 / 人間発)
- [x] 4.5 テスト(調停決定、バイパス配送、合議プロトコル、human timeout)

## Wave 5: API + WebUI(§10, §11)
- [x] 5.1 REST エンドポイント6種 + instruction_received 配送
- [x] 5.2 `vsm instruct` CLI
- [x] 5.3 topology projection API
- [x] 5.4 React「組織図」ビュー(Node カード、状態色分け、予算バー、介入操作)
- [x] 5.5 ドキュメント更新(README 起動手順一本化、implementation-status.md、ROADMAP.md)
