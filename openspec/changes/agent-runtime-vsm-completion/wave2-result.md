...
- AgentRuntime 呼出前に、直近実績と設定済み初期値の大きい方へ安全倍率を掛け、Node/Run の
  残余が見積未満なら呼び出さず既存の budget escalation へ送る。
- ロール別 envelope が未指定の Node は Run envelope を Node envelope として使う。
- quota 応答も返却された利用量を `budget_consumed` に記録してから suspend する。
- quota は `five_hour` / `weekly` と正確な reset 時刻を quota-state v2 に保存する。
  unknown または時刻不明は `human_review_required` としてfail-fastし、自動復帰しない。
...
- Budget/quota + merge integration: `12 passed`
- quota 種別解析・永続復元: `4 passed`
- backend quota 結果正規化: `2 passed`
