...
- Human は詳細の `in_doubt_effects`（effect_id、種別、入力hash、発生時刻）を確認し、外部事実ありを`completed`、無かったを`failed`として理由付きで裁定できる。completed は `tool_completed(recovered=true)`、failed は `tool_failed(disposition=human_decision)` を durable に追記し、裁定対象に紐づく recovery pause を全て解除する。裁定がない限り force abort は受け付けない。
...
| `POST /api/selfdev/proposals/{proposal_id}/control` | abort/suspend/resume（`pause_id`指定可）/force_abort |
| `POST /api/selfdev/proposals/{proposal_id}/human-decision` | approve/reject/respond、in-doubt effect の completed/failed 裁定 |
...
```json
{
  "action": "suspend | resume | abort | force_abort",
  "reason": "...",
...
`pause_id`は`resume`時だけ任意指定できる。複数pauseで省略した場合は409。`force_abort`は全in-doubt裁定後に限り、cleanupを実行せず既存artifact参照を記録してterminal化するHuman専用の最後の脱出弁である。controllerがcommandをdurable受理したら`202`。安全停止・patch保存等が完了するまで「suspended/aborted」とは返さない。

### Human decision

```json
{
  "decision": "approve | reject | respond | completed | failed",
  "reason": "...",
  "statement": null,
...
`completed`/`failed`では`effect_id`と理由が必須で、詳細の `in_doubt_effects` にある効果だけを裁定できる。protected approveでは両hash必須。actorは認証済みHumanでなければならず、FableをHumanとして扱わない。
