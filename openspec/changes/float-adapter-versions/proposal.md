...
- **CHANGED(EEP 整合):** `add-execution-environment-profiles`(EEP-01 の「要求 CLI バージョン」)は、本 change の方針に合わせて「最低要求版(任意)」へ弱める。ただし本 change では EEP の spec 本文は変更しない。実装統合時に EEP 側の該当項目を最低要求版へ調整する旨を `tasks.md` に残す。
