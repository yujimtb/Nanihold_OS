...
### 実施形態の補足(オーナー承認 2026-07-22・実施済み)

適用 26 セルの検証は次の二層で実施した。

1. **内部経路の等価シミュレーション**: `InboundMessage` 境界注入により、実宛先解決・実 LETHE・実 Operational Ledger を通して全 26 セルを検証する層。モック単体テストではなく、実コンポーネント・実データレイクを通す実測である。
2. **実 Discord / 実 Slack 画面からの縮約実測**: 層1の等価シミュレーションでは覆えない Gateway 境界(実チャネル → 宛先解決 → 通知)と実外部送信(承認 → 実送信)のみを対象とする。`intercom/addressing.py` の `AddressResolver.resolve()` は宛先個名の種別(`Nagi` 固定席かタスクエージェントか)で処理経路が分岐しないため、Gateway 境界においては両者が同一処理であり、片方(`Nagi`)の代表実施でもう片方を構造的に等価として扱う。縮約セットは各チャネル(Discord / Slack)× 宛先解決5種の代表 + 返信承認 → 実送信の代表、計12項目。

「モック単体はこれに代替してはならない(SHALL NOT)」の原則は変更しない。

代表セル対応表: `docs/acr08-owner-checklist-reduced.md`。実施記録(Ledger イベント・監査トレース等のエビデンス): `runs/acr08/`。
