...
- **ADDED:** ACR-08 実経路疎通試験マトリクス — 受け入れの最終条件として、チャネル(Discord/Slack)× 宛先解決(5 モード)× 宛先種別(Nagi/タスク実行エージェント)× 方向(3)の直積(総 60 セル)を実 Discord/Slack/LETHE/Nanihold の実経路で end-to-end 疎通確認。自動化可能セルはスクリプト化、人間操作が必要なセルはオーナー向け手順チェックリスト(送る文言・期待結果付き)として成果物化。
...
8. **実経路疎通試験マトリクス(ACR-08 新規)**: 受け入れ最終条件として、チャネル×宛先解決×宛先種別×方向の直積(総 60 セル)を実 Discord/Slack/LETHE/Nanihold で end-to-end 疎通確認。適用不能セルは N/A 理由明記。自動化可能セルはスクリプト化、人間操作セルはオーナー向け手順チェックリスト成果物化 → オーナー実施 → システム側で自動検証。
