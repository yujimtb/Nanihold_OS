...
PilotHostは`nh-control-a`と`nh-control-b`へ1台ずつ固定します。generic Interface
PilotはClaude provider側のモデル設定と`high` effortを使い、人格名やモデル固有名を
Naniholdの識別子へ含めません。coding S1はdeployment inputの明示exact model／effort
...
- Claude Interface PilotのCLI version、`model_selection=provider_configured`、
  `effort=high`、budget、timeout
- coding PilotのCLI version、exact model、exact effort、budget、timeout
