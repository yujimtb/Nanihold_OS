...
指示できます。受理した指示は対象 Node の次の LLM invocation 開始前にプロンプトへ注入され、
`instruction_applied` が対象 invocation ID とともに記録されます。実行中の invocation 自体は
割り込みません。最近の Run は `vsm runs` で一覧できます。
