...
永続する対話主体はユーザー所有の Interface Node です。Fable は現在そこへ接続する Interface Pilot の一つであり、設定既定値は `claude-fable-5 / high` です。Pilot が変わっても Node、会話、決定、約束、未完 WorkItem は変わりません。
...
- Token Lab の試験はロジック優先です。モデルが必要な場合も安価なモデルの `low` のみで、Fable と Opus は禁止します。
