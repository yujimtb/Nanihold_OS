...
```powershell
docker compose -p eep-runbook run --no-deps --rm app python3 -c "from vsm.environment import EnvironmentContract, environment_fingerprint; from vsm.environment_instance import compute_instance_fingerprint; c=EnvironmentContract.model_validate({'supported_shells':['powershell','posix'],'workspace_writable':True,'minimum_memory_mb':4096,'supported_sandboxes':['workspace-write'],'required_sandbox':'workspace-write','path_mapping_names':['workspace-root'],'adapters':{'codex-cli':{'required_endpoints':['api.openai.com'],'minimum_cli_version':'0.144.5'},'claude-code':{'required_endpoints':['api.anthropic.com'],'minimum_cli_version':'2.1.215'}}}); print('environment_contract='+environment_fingerprint(c)); print('instance='+compute_instance_fingerprint(logical_path_bindings={'workspace-root':'D:/userdata/docs/projects'},cli_executable_path='C:/Users/mitob/AppData/Roaming/npm/node_modules/@openai/codex/node_modules/@openai/codex-win32-x64/vendor/x86_64-pc-windows-msvc/bin/codex.exe',codex_home='C:/Users/mitob/.codex',environment_variables={},machine_identity={'execution':'windows-native','os':'windows'}))"
```
...
```text
environment_contract=environment-contract-sha256:094cd46feadc4ea089448f5c944cd3b2c62e38cf1f6863d9e4d0816382006c06
...
preflight_cli_version_files = { "codex-cli" = 'C:\Users\mitob\AppData\Roaming\npm\node_modules\@openai\codex\package.json', "claude-code" = 'C:\Users\mitob\AppData\Roaming\npm\node_modules\@anthropic-ai\claude-code\package.json' }
...
`preflight_cli_version_files` と `preflight_cache_path` は、TOMLの単一引用リテラルで
Windowsバックスラッシュをそのまま記載します。API (Linux container) 側の expectation
は `str(Path)` の結果を素通しし、PilotHost (Windows) の health は `WindowsPath` の
バックスラッシュ正規化値を返すためです。`C:/Users/...` のようなフォワードスラッシュ、
または同じ見た目にするためのTOML基本文字列のエスケープへ変更してはいけません。
...
```toml
environment_fingerprint = "environment-contract-sha256:094cd46feadc4ea089448f5c944cd3b2c62e38cf1f6863d9e4d0816382006c06"
```
...
```powershell
python -c "from pathlib import Path; from vsm.environment import EnvironmentContract; from vsm.environment.artifacts import LocalEnvironmentContractStore; c=EnvironmentContract.model_validate({'supported_shells':['powershell','posix'],'workspace_writable':True,'minimum_memory_mb':4096,'supported_sandboxes':['workspace-write'],'required_sandbox':'workspace-write','path_mapping_names':['workspace-root'],'adapters':{'codex-cli':{'required_endpoints':['api.openai.com'],'minimum_cli_version':'0.144.5'},'claude-code':{'required_endpoints':['api.anthropic.com'],'minimum_cli_version':'2.1.215'}}}); a=LocalEnvironmentContractStore(Path(r'D:/userdata/docs/projects/_cutover_20260720_fable_activation/production/environment-artifacts')).save(c,artifact_key='codex-workspace',version=1); print(a.fingerprint)"
```

**期待される出力 / 状態**

`environment-contract-sha256:094cd46f...006c06` が表示され、
...
```json
{
  ...
  "preflight": {
    ...
    "cli_version_files": {
      "codex-cli": "C:\\Users\\mitob\\AppData\\Roaming\\npm\\node_modules\\@openai\\codex\\package.json",
      "claude-code": "C:\\Users\\mitob\\AppData\\Roaming\\npm\\node_modules\\@anthropic-ai\\claude-code\\package.json"
    },
...
`preflight.data_space_id` は必須です。`environment_instance` は次の6フィールドだけを
持つ厳密なフィールド集合で、例では6個すべてを明示しています。
`instance_id`、`logical_path_bindings`、`cli_executable_path`、`codex_home`、
`environment_variables`、`machine_identity` 以外を追加してはいけません。特に
`data_space_id` は `environment_instance` 内ではなく `preflight` 直下に置きます。
欠落時は `environment instance DataSpace ID must be a non-blank string`、余剰フィールドは
`fields differ from the exact contract` で拒否されます。
...
実際のhealthにはadapter別CLI version filesとcache pathも含まれ、`environment_fingerprint` は
各 adapter candidateと厳密一致する。
...
bridge有効時のpreflightは通常時と異なります。Codex rolloutは
`sandbox_policy` を報告しないため、`ad918c8` 以降のpreflightはこのbridgeを検出すると
`sandbox_policy = "unverified_by_bridge"`、`evidence.bridged = true` を記録した縮退合格に
します。これはsandboxを検証済みとみなすfallbackではなく、bridge中は独立検証できないことを
明示した合格です。shell、CLI version、endpoint、memory、workspace writableなど他の検証は
引き続き必須です。`ad918c8` より前のコードでは同じ欠落が全実行拒否になるため、bridgeを
有効にしたまま旧コードを起動してはいけません。

**期待される出力 / 状態**

bridge中だけ、Codexのsandbox policy欠落が明示的な縮退合格として記録され、Windows
workspace-writeのdispatchが `preflight is None` のfail-closed条件で全停止しない。
bridge終了後のhealth/configには `preflight.enabled=true` と
`win32_codex_sandbox_bypass_enabled=false` が残る。
...
## 5. RouteSnapshot遷移（2026-07-22成功手順、必須）
...
旧手順の「旧serveを稼働させたままAPIでretire/publish」は成立しません。サービング中のAPIは
外部書き込みの台帳イベントをブート時にしか読み込まないため、CLIがregister/approveした
新snapshotを旧serveは知りません。さらにレジストリは設定由来なので、旧設定serveは新候補を、
新設定serveは旧snapshotを扱えません。したがって、この章ではAPIのsnapshot操作endpointを
使わず、次の順序を厳守します。

1. PilotHostを新設定で再起動してhealthを確認してから、`vsm routes publish` を実行する。
2. PilotHost/APIを旧設定へ一時復帰する。
3. API container内で、遷移設定を使うkernel bootstrapスクリプトを実行し、台帳へ旧snapshotの
   retireと新snapshotのpublishを書き込む。
4. PilotHostを新設定へ戻し、`vsm.toml` を新active IDへ変更してからAPIを最終再起動する。

### 5.1 遷移設定と共通値を準備する

**実行するコマンド / 編集内容**

3〜4章で作成した新設定を、旧serveを停止・再作成せずに実行するための一時設定へ保存します。
`TransitionConfig` は新contract、artifact、instance、preflightと新候補を持ち、
`routing.active_route_snapshot_id` は新IDでなければなりません。稼働中の旧serveはこの時点で
旧 `vsm.toml` と旧active IDのままです。
...
$S3StarActorId = "actor:independent-s3-star"
...
# TransitionConfigには3.1〜3.2の新設定を反映し、active IDだけをNewSnapshotIdにする。
Select-String -LiteralPath $TransitionConfig -Pattern "active_route_snapshot_id|environment-contract-sha256:094cd46f|preflight_enabled = true"
```

`$ApiHeaders`、`$PilotHeaders`、`$ApiContainer`、PilotHost起動用のcommission済み引数は、
以降の全操作で同じ値を使います。`TransitionConfig` はAPI containerから
`/cutover/production/vsm.eep08-transition.toml` として見える場所に置き、worktreeやgitへ
commitしません。

**期待される出力 / 状態**

旧serveは旧IDでreadyのまま、TransitionConfigは新fingerprintと
`active_route_snapshot_id = "route:coding-production-20260722-eep08-luna-first"` を持ちます。

**失敗時の判断**

旧serveがreadyでない、またはTransitionConfigのfingerprint・active IDが不一致なら、新設定の
PilotHostを起動しません。旧configのbackupを使って状態を復旧し、原因を解消します。

### 5.2 PilotHostを新設定へ戻してregister/approveまで実行する

**実行するコマンド / 編集内容**

`vsm routes publish` はPilotHost healthを検証するため、先に4.1の新しい
`pilot-host.json` を配置してPilotHostだけを新設定で再起動します。PilotHostの起動は、
標準の `production/start-pilot-host.ps1`（標準出力・標準エラーのリダイレクトとPID receipt
付き）を使います。

```powershell
$PilotHostStart = Join-Path $RepositoryRoot "production/start-pilot-host.ps1"
& $PilotHostStart `
  -PythonExecutable $PilotHostPythonExecutable `
  -RepositoryRoot $RepositoryRoot `
  -ConfigFile $PilotHostConfig `
  -RuntimeEnvFile $RuntimeEnvFile `
  -LogFile $PilotHostLog `
  -PidFile $PilotHostPidFile `
  -ReadyTimeoutSeconds 60

$Health = Invoke-RestMethod -Method Get -Uri "$PilotHostBase/health" -Headers $PilotHeaders
if ($Health.preflight.enabled -ne $true) { throw "新PilotHostのpreflightがenabledではありません" }
...
新fingerprintを含むLunaとSolのcandidate keyがそれぞれ1件だけ得られ、CLIはregister、
S3* approve、owner approveまで到達します。その後、旧snapshotがまだPUBLISHEDなので
publish段階で停止します。これは期待どおりの停止です。この直後に旧serveのAPIで新IDを
read-backしてはいけません。旧serveはCLIが書いたregister/approveイベントをまだ投影して
いないためです。

**失敗時の判断**

register/approve前の失敗なら、evidence cursor、candidate key、artifact、configを直して停止します。
新IDが既に台帳へ書かれている場合は `vsm routes publish` 全体を再実行せず、5.3の旧設定復帰へ
進みます。新IDが台帳に存在しない場合だけpayloadとエラーを保全してowner/S5判断を得ます。

### 5.3 PilotHost/APIを旧設定へ一時復帰する

**実行するコマンド / 編集内容**

5.2の新PilotHostを停止し、2.1で取得したbackupからPilotHost設定を戻します。続けて
`vsm.toml` も旧backupへ戻し、旧candidate registryと旧active IDでAPIを再起動します。
API起動時はPilotHostとAPIのcandidateが一致していなければならないため、両方を旧設定へ
戻してからAPIを起動します。
...
# PilotHostを停止後、production/start-pilot-host.ps1を旧pilot-host.jsonで実行する。
# その後、既存のAPI起動手順で旧vsm.tomlのAPIを再起動し、旧health/readyを確認する。
```

**期待される出力 / 状態**

PilotHostとAPIが旧candidate、旧active IDでreadyになります。新設定の候補を持たない旧APIを
稼働させたまま新設定へ切り替えたり、旧APIのsnapshot endpointでretire/publishを実行したり
しません。

**失敗時の判断**

PilotHost/APIのcandidateが一致しない、または旧設定でreadyにならない場合はkernel bootstrapを
実行しません。backupと5.2のCLI実行結果を保全して停止します。

### 5.4 kernel bootstrapで台帳のretire/publishを実行する

**実行するコマンド / 編集内容**

新設定と新active IDを持つ `TransitionConfig` を使い、`require_active_route=False` で
`vsm.cli.bootstrap` を起動します。API container内で新しいRuntimeを一時起動し、そのRuntimeの
kernelへ旧snapshotのretire、新snapshotのpublishをこの順で直接記録します。APIの常駐serveの
kernelやHTTP endpointは使いません。

```powershell
$TransitionScript = @'
from pathlib import Path
import vsm.cli
from vsm.kernel.models import RouteSnapshotRetirementReason

runtime = vsm.cli.bootstrap(
    Path("/cutover/production/vsm.eep08-transition.toml"),
    require_active_route=False,
)
try:
    runtime.kernel.retire_route_snapshot(
        "route:coding-production-20260720-luna-first",
        reason_code=RouteSnapshotRetirementReason.SUPERSEDED_BY_APPROVED_SNAPSHOT,
        replacement_snapshot_id="route:coding-production-20260722-eep08-luna-first",
        actor_id="owner:primary",
        idempotency_key="eep08:route:20260722:kernel-retire-old",
    )
    runtime.kernel.publish_route_snapshot(
        "route:coding-production-20260722-eep08-luna-first",
        actor_id="owner:primary",
        idempotency_key="eep08:route:20260722:kernel-publish-new",
    )
finally:
    runtime.close()
'@
$TransitionScript | docker exec -i $ApiContainer python -
```

`TransitionConfig` の `active_route_snapshot_id` は新IDのまま、`require_active_route=False` で
実行します。これにより、起動時にactive routeがまだPUBLISHEDでない遷移中でも、台帳から旧・新
snapshotを投影して同一kernel上で順序どおりにイベントを書けます。

**期待される出力 / 状態**

台帳上で旧snapshotが `RETIRED`（reasonは
`superseded_by_approved_snapshot`）、新snapshotが `PUBLISHED` になります。旧APIの常駐
processはこの書き込みを即時には知らないため、成功判定は台帳のread-backまたは次のAPI再起動後
に行います。

**失敗時の判断**

どちらか一方のイベントだけが書かれた可能性がある場合、スクリプトを無条件に再実行しません。
台帳のsnapshot state、event、idempotency keyを保全し、未完了のstateから再開方法をowner/S5と
決めます。active route検証を通すために `require_active_route=True` へ変更してはいけません。

### 5.5 PilotHostを新設定へ戻し、active IDを反映してAPIを最終再起動する

**実行するコマンド / 編集内容**

kernel bootstrapのread-backで旧RETIRED・新PUBLISHEDを確認した後、4.1の新
`pilot-host.json` を配置し、`production/start-pilot-host.ps1` でPilotHostを先に起動します。
次に正本 `vsm.toml` を新設定へ戻し、`routing.active_route_snapshot_id` が新IDであることを
確認してからAPIを最終再起動します。
...
PilotHost healthが新fingerprintとpreflight設定を返し、APIの最終bootstrapが新PUBLISHED
snapshot、設定由来の新candidate registry、current evidence cursorを同時に検証してreadyに
なります。`TransitionConfig` は成功確認後もmaintenance receiptとともに保全します。

**失敗時の判断**

`active RouteSnapshot references an unregistered ModelCandidate`、stale evidence、
`production PilotHost preflight configuration mismatch` のいずれかならdispatchを再開しません。
直前backup、kernel bootstrapのevent、PilotHost healthを保全し、5.2〜5.5の順序と両設定の
candidate一致を確認します。
...
# 4. production/start-pilot-host.ps1（リダイレクト付き）を5.2と同じcommission済み引数で実行し、認証付きhealthを取得
Invoke-RestMethod -Method Get -Uri "$PilotHostBase/health" -Headers $PilotHeaders
# 5. 既存のAPI起動手順でNanihold APIを再作成しreadyを確認
...
if ($Health.preflight.environment_fingerprint -ne "environment-contract-sha256:094cd46feadc4ea089448f5c944cd3b2c62e38cf1f6863d9e4d0816382006c06") { throw "PilotHost contract fingerprintが不一致です" }
if ($Health.preflight.cli_version_files.'codex-cli' -ne 'C:\Users\mitob\AppData\Roaming\npm\node_modules\@openai\codex\package.json') { throw "Codex version fileのWindows pathが不一致です" }
if ($Health.preflight.cache_path -ne 'D:\userdata\docs\projects\_cutover_20260720_fable_activation\production\preflight-cache.json') { throw "preflight cacheのWindows pathが不一致です" }
```

**期待される出力 / 状態**

healthの `preflight.enabled=true`、contract/instance fingerprint、CLI version file、cache
pathがTOMLのWindowsバックスラッシュ文字列と一致する。実際のpreflight evidenceは次の6.3の
dispatchで生成する。
...
$PreflightCache = Join-Path $ProductionRoot "preflight-cache.json"
if (!(Test-Path -LiteralPath $PreflightCache -PathType Leaf)) { throw "preflight-cache.json が生成されていません" }
$Cache = Get-Content -Raw -LiteralPath $PreflightCache | ConvertFrom-Json
if (!$Cache.entries -or $Cache.entries.Count -lt 1) { throw "preflight cacheにevidenceがありません" }
```

**期待される出力 / 状態**

Codexが通常の `--sandbox workspace-write` で起動し、Pilot receipt、actual model、
preflight evidence、Ledger eventが保存される。2026-07-22の成功例では、検証WorkItemを
dispatchしてから15秒以内に受入され、`preflight-cache.json` が生成された。Lunaから開始し、
Solは明示的なcoding escalation条件のときだけ使われる。
...
registryを含む旧configをcommission用の遷移設定へ戻し、5.4と同じくAPI container内の
`vsm.cli.bootstrap(..., require_active_route=False)` からkernelへ新snapshotのretireと旧snapshot
のpublishを記録します。常駐APIが古いprojectionのままの状態でHTTP endpointを呼ばず、台帳の
stateを確認してから旧active IDでserveを再作成します。
...
`production/start-pilot-host.ps1` で旧PilotHost → 旧APIの順に再起動し、6.1のhealth/readiness、6.2のpreflight、6.3の最小
...
- [ ] 初回 `[environment_contract]` の共通能力要件と `adapters` 要件集合が実装スキーマと一致する
...
- [ ] `preflight.data_space_id` があり、`environment_instance` が指定6フィールドだけである
- [ ] `preflight_enabled=true` とPilotHost `preflight.enabled=true` が一致する
- [ ] `win32_codex_sandbox_bypass_enabled=false` が通常状態で明示される
- [ ] bypass=trueを使った場合、承認・期限・false復帰のreceiptと
      `preflight-cache.json` の `evidence.bridged=true` がある
- [ ] coding candidateはLunaとSolの正確な2件で、RouteSnapshot順序もLuna→Solである
- [ ] 新RouteSnapshotがregister、S3* approve、owner approve済みである
- [ ] 新PilotHostでCLI publishを実行し、旧PUBLISHEDによるpublish段停止を確認した
- [ ] 旧PilotHost/APIへ戻した後、API containerのkernel bootstrapで旧RETIRE・新PUBLISHを記録した
- [ ] `production/start-pilot-host.ps1` で新PilotHostを起動し、active IDを新設定にしてAPIを最終再起動した
- [ ] `routing.active_route_snapshot_id` と新PUBLISHED snapshotが一致する
- [ ] `preflight-cache.json` と `environment_instance_preflight_verified` Eventがある
- [ ] 2026-07-22成功例どおり、検証WorkItemのdispatchから15秒以内の受入を確認した
