...
- Nanihold最終差分全体: 119 passed。revision retry専用Event、
  `reorientation_attempt_in_progress`再構築、二重start拒否、runtime restart時の
  明示interruption Eventを含む
...
`work:interface-effective-activity-start`と一致する。状態は
`AWAITING_OWNER_CONFIRMATION`であり、`ACTIVE`ではない。Execution、Effect Lease、
BudgetReservationは0で、owner承認前の副作用は発生していない。
...
- 修正版Assessmentはowner確認待ち。訂正または明示承認なしに`ACTIVE`へ進めない
- owner confirmation前に`ACTIVE`、Execution、Effectへ進めない
...
- production deployとInterface Nodeの`ACTIVE`化は未実施
