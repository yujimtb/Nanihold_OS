...
`[residency].native_runs_enabled` は既定 `false` で、無効時のRun作成・再開系APIは HTTP 403 と
`D0契約によりnative Runは封鎖中` を返す。画面のRun作成欄にも封鎖理由を表示する。

## 操作

トップバーの「事業状況」は技術的な LiveTopology と分離した経営 dashboard である。円建て台帳から
available cash、30日 burn、runway、R、未価格化 usage、日次推移を表示する。正本 API は
`GET /api/survival/dashboard`、手入力の収支は `POST /api/survival/ledger` で登録する。
外部送信・実請求は無効で、Human 認証は owner 承認待ちの placeholder 表示に留める。
...
- **このメッセージをRunとして実行**: `POST /api/runs` に本文を `goal` として投入する。native Run
  が封鎖中なら、画面はAPIの403理由をそのまま表示する。
...
| `GET /api/survival/dashboard` | 円建て cash/burn/runway/R と30日推移、基線、安全境界を返す |
| `GET /api/survival/ledger` | append-only ledger と usage の現在スナップショットを返す |
| `POST /api/survival/ledger` | 人手の売上・支出・オーナー拠出を記録する（実請求はしない） |
