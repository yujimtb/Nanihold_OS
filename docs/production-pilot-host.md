...
`preflight` は任意の top-level field です。省略時は既存の PilotHost 起動・実行経路を
維持します。契約適合ゲートを有効にする場合は、次の全フィールドを明示します。
...
`cli_version_file` は `package.json` の `version` を読むか、JSON 以外ならファイル
全体を版文字列として読むパスです。PilotHost はこのファイルを毎回 `stat` と読み取り
...
`environment_fingerprint(contract)` で計算し、Codex candidate の宣言値と厳密に
...
dispatch 直前の検証タプルは `(cli_version, required_sandbox,
environment_fingerprint)` です。版ファイルの mtime もキャッシュ整合性に含め、
タプルと mtime が一致した dispatch では Codex preflight 試走を省略します。不一致時は
Codex を1回試走し、rollout の `sandbox_policy` を読み、契約に合格した場合だけ実行へ
進みます。

`vsm.preflight.PreflightGate` の接続面は次のとおりです。

- `preflight_runner(verification_tuple) -> PreflightObservation | Mapping` は試走と
...
本番ランチャーからこのLedger hookを生成するには、commission済みDataSpace、
EnvironmentInstance ID、Operational Ledger接続を設定へ追加する必要があります。
現時点のJSONにはこれらが無いため、`ProductionPilotHost` の外側でhookを注入します。
同様に、組み込みCodex試走が直接読めるのはrollout sandboxとworkspace modeまでです。
endpoint・memory・shell・logical pathの実測はEnvironmentInstance束縛から構築した
instance-aware runnerを注入する必要があり、未注入で証拠が欠ける場合はfail-fastします。
