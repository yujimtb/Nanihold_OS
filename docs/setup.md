...
```toml
[residency]
# D0契約では native Run を既定で封鎖する。起動する場合だけ true にする。
native_runs_enabled = false
...
Run作成、失敗Runの再試行など native Run の起動・再開が起動前に拒否されます。REST API のRun作成・
再開系は HTTP 403 と `D0契約によりnative Runは封鎖中` を返します。chat API、selfdev、既存Runの
読み取り、suspend/terminate/cancel などの停止操作は利用できます。native Run を明示的に有効化する
場合は次を設定します。
